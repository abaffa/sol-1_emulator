...
void BAFFA1_MEMORY::refresh_int_pending(BAFFA1_MICROCODE& microcode, BAFFA1_ALU& alu, BAFFA1_REGISTERS& registers) {
	microcode.controller_bus.int_pending = get_byte_bit(microcode.controller_bus.int_request, 0) & get_byte_bit(alu.MSWl.value(), MSWl_INTERRUPT_ENABLE);
}


void BAFFA1_MEMORY::refresh_any_interruption(BAFFA1_MICROCODE& microcode) {
	microcode.controller_bus.any_interruption = get_byte_bit(microcode.controller_bus.int_pending, 0) | get_byte_bit(microcode.controller_bus.dma_req, 0);
}
///////////////////
...
void BAFFA1_MEMORY::refresh_pagetable_mem(BAFFA1_MICROCODE& microcode, BAFFA1_ALU& alu, BAFFA1_REGISTERS& registers, BAFFA1_CONFIG& config, FILE *fa) {
	...
	if (microcode.controller_bus.pagtbl_ram_we && microcode.controller_bus.mdr_to_pagtbl_en)
	{
		...
		this->pagtbl_ram_l.write(pagtbl_ram_addr, registers.MDRl.value());
		this->pagtbl_ram_h.write(pagtbl_ram_addr, registers.MDRh.value());
	}
}

unsigned long BAFFA1_MEMORY::read_address_bus(BAFFA1_BYTE bus_tristate, BAFFA1_MICROCODE& microcode, BAFFA1_ALU& alu, BAFFA1_REGISTERS& registers) {
	unsigned long address_bus = 0x00;

	if (bus_tristate == 0x00) {

		if (!get_byte_bit(alu.MSWl.value(), MSWl_PAGING_EN)) {
			address_bus = (unsigned long)((registers.MARh.value() << 8) | registers.MARl.value());
			...
			if (!microcode.controller_bus.mdr_to_pagtbl_en) {
				...
				microcode.controller_bus.memory_io = get_byte_bit(this->pagtbl_ram_h.read(pagtbl_ram_addr), 3);
				microcode.controller_bus.page_present = get_byte_bit(this->pagtbl_ram_h.read(pagtbl_ram_addr), 4);
				microcode.controller_bus.page_writable = get_byte_bit(this->pagtbl_ram_h.read(pagtbl_ram_addr), 5);
			}
			else {
				mem_table_addr = registers.MDRl.value();
				mem_table_addr |= (registers.MDRh.value() & 0b00000111) << 8;
	...
	if (bus_tristate == 0x00) {
		...
		if (!get_byte_bit(alu.MSWl.value(), MSWl_PAGING_EN)) {
			...
			if (!microcode.controller_bus.mdr_to_pagtbl_en) {
				...
				if (page_table_addr_src(microcode.controller_bus.force_user_ptb, alu.MSWl.value()))
					ptb_mem_addr = ptb_mem_addr | registers.PTB.value() << 5;


				mem_table_addr = this->pagtbl_ram_l.read(ptb_mem_addr);
				mem_table_addr |= ((this->pagtbl_ram_h.read(ptb_mem_addr) & 0b00000111) << 8);

				microcode.controller_bus.memory_io = get_byte_bit(this->pagtbl_ram_h.read(ptb_mem_addr), 3);
				microcode.controller_bus.page_present = get_byte_bit(this->pagtbl_ram_h.read(ptb_mem_addr), 4);
				microcode.controller_bus.page_writable = get_byte_bit(this->pagtbl_ram_h.read(ptb_mem_addr), 5);
			}
			else {
				mem_table_addr = registers.MDRl.value();
				mem_table_addr |= ((registers.MDRh.value() & 0b00000111) << 8);
